...
func (b *Builder) build(ctx context.Context) (*Node, error) {
	...
	// Review: this is only used in setting up the PorcelainAPI, find a home, network probably
	// set up pinger
	pingService := ping.NewPingService(nd.Network.host)
	...
	nd.PorcelainAPI = porcelain.New(plumbing.New(&plumbing.APIDeps{
		...
		MsgQueryer:   msg.NewQueryer(nd.Chain.ChainReader, nd.Blockstore.cborStore, nd.Blockstore.Blockstore),
		MsgWaiter:    msg.NewWaiter(nd.Chain.ChainReader, nd.Chain.MessageStore, nd.Blockstore.Blockstore, nd.Blockstore.cborStore),
		// XXX: this thing is not pumbling, it should not be created direcrlt on porcelain
		Network:       net.New(nd.Network.host, pubsub.NewPublisher(nd.Network.fsub), pubsub.NewSubscriber(nd.Network.fsub), net.NewRouter(nd.Network.Router), bandwidthTracker, net.NewPinger(nd.Network.host, pingService)),
...
func (b *Builder) buildBlockstore(ctx context.Context) (BlockstoreSubmodule, error) {
	...
	// spoof ipldCbor to point to local store
...
func (b *Builder) buildChain(ctx context.Context, blockstore *BlockstoreSubmodule, network *NetworkSubmodule) (ChainSubmodule, error) {
	...
	nodeConsensus := consensus.NewExpected(blockstore.cborStore, blockstore.Blockstore, processor, blkValid, powerTable, b.genCid, b.BlockTime, consensus.ElectionMachine{}, consensus.TicketMachine{})
